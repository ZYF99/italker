package com.zyf.italker.factory.utils;

import android.support.v7.util.DiffUtil;

import java.util.List;

public class DiffUiDataCallback<T extends DiffUiDataCallback.UiDtaDiffer<T>> extends DiffUtil.Callback {
    private List<T>mOldList,mNewList;

    public DiffUiDataCallback(List<T> mOldData, List<T> mNewList) {
        this.mOldList = mOldData;
    ...
    public boolean areItemsTheSame(int i, int i1) {
        T beanOld = mOldList.get(i);
        T beanNew = mNewList.get(i1);
    ...
    public interface UiDtaDiffer<T>{
        //传递一个旧的数据给你，问你是否表示的是同一个数据
